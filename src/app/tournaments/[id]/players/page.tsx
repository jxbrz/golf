...
import { getDbFieldLeaderboard } from "@/lib/db-data/entries";
...
export default async function PlayersLeaderboardPage({
  ...
  const dbField = await getDbFieldLeaderboard(tournament.id);
  const field = dbField.length ? dbField : getFieldLeaderboard(tournament.id);
  const allGolfers = dbField.length ? dbField : getTournamentGolfers(tournament.id);
